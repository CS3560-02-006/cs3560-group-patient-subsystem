...
def updatePatient(request, patient_id):
    ...
    street = replace_empty_with_none(
        address_data.get('street')) if address_data else None
    apt = replace_empty_with_none(
        address_data.get('apt')) if address_data else None
    city = replace_empty_with_none(
        address_data.get('city')) if address_data else None
    state = replace_empty_with_none(
        address_data.get('state')) if address_data else None
    zipcode = replace_empty_with_none(
        address_data.get('zipcode')) if address_data else None
    ...
    with connection.cursor() as cursor:
        if update_fields:
            ...
            cursor.execute(
                f"UPDATE Patient SET {set_clause} WHERE patientID=%s", set_values)
...
def deletePatient(request, patient_id):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(
            "UPDATE Appointment SET patientID=NULL, status='available' WHERE patientID=%s", [patient_id])
